...
class Rutina(
    ...
    var listaActividades: Array<Ejercicio> = emptyArray()
) {
...
fun main() {
    val usuarios = Array<Usuario?>(100) { null }
    var menuPrincipal = 0
    ...
    while (menuPrincipal != 3) {
        ...
        when (menuPrincipal) {
            1 -> {
                ...
                // Buscar la primera posición disponible en el array de usuarios
                val index = usuarios.indexOfFirst { it == null }

                if (index != -1) {
                    usuarios[index] = Usuario(nombre, correo, contraseña, edad, genero, masaCorporal, altura)
                    println("Usuario registrado exitosamente.")
                } else {
                    println("No hay espacio disponible para registrar un nuevo usuario.")
                }
            }
            2 -> {
                // Ingresar usuario
                ...
                usuario = usuarios.find { it?.correo == correo && it?.contraseña == contraseña } ?: null
